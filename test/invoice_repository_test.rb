...
class InvoiceRepositoryTest < Minitest::Test
  ...
  def setup
    ...
    @invoice_repository = InvoiceRepository.new(nil, "./fixtures/invoices_testdata.csv")
    invoice_repository.load_file
  end

  def test_load_test_datafile
    assert_equal 24, invoice_repository.all.length
  end

  def test_the_1st_record_has_customer_id_
    assert_equal 1, invoice_repository.all[0].customer_id
  end
  ...
  def test_the_10th_record_has_status_shipped
    assert_equal "shipped", invoice_repository.all[11].status
  end
