...
class ItemRepositoryTest < Minitest::Test
  ...
  def setup
    ...
    @item_repository = ItemRepository.new(nil, "./fixtures/items_testdata.csv")
    item_repository.load_file
  end

  def test_load_test_datafile
    assert_equal 19, item_repository.all.length
  end

  def test_the_1st_record_has_name_Item_Qui_Esse
    assert_equal "Item Qui Esse", item_repository.all[0].name
  end

  def test_the_1st_description_is_Nihil
    assert_equal "Nihil autem sit odio inventore deleniti. Est laudantium ratione distinctio laborum. Minus voluptatem nesciunt assumenda dicta voluptatum porro.", item_repository.all[0].description
  end

  def test_the_3rd_unit_price_is_32301
    assert_equal 323.01, item_repository.all[2].unit_price
  end

  def test_the_4th_merchant_id_is
    assert_equal 1, item_repository.all[3].merchant_id
  end
  ...
  def test_find_all_by_unit_price
    assert_equal 1, item_repository.find_all_by_unit_price(BigDecimal("594.54")).count
  end
