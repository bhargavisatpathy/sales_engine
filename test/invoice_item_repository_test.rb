...
class InvoiceItemRepositoryTest < Minitest::Test
  ...
  def setup
    ...
    @invoice_item_repository = InvoiceItemRepository.new(nil, "./fixtures/invoice_items_testdata.csv")
    invoice_item_repository.load_file
  end

  def test_load_test_datafile
    assert_equal 22, invoice_item_repository.all.length
  end
  ...
  def test_find_by_invoice_id_has_invoice_id_1
    assert_equal 1, invoice_item_repository.all[2].invoice_id
  end

  def test_find_by_item_id
    assert_equal 535, invoice_item_repository.all[3].item_id
  end

  def test_find_by_quantity_has_quantity_5
    assert_equal 8, invoice_item_repository.all[2].quantity
  end

  def test_find_by_unit_price_has_value_of_34873
    assert_equal BigDecimal("348.73"), invoice_item_repository.all[2].unit_price
  end
  ...
  def test_find_all_by_unit_price
    assert_equal 3, invoice_item_repository.find_all_by_unit_price(BigDecimal("720.18")).count
  end
